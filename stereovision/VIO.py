...
class VisualOdometry():
    def __init__(self, data_dir):
        ...
        self.fastFeatures = cv2.FastFeatureDetector_create(threshold=12)
    ...
    def reprojection_residuals(self, dof, q1, q2, Q1, Q2):
        ...
        # Homogenize and project
        ones = np.ones((q1.shape[0], 1))
        Q1 = np.hstack([Q1, ones])
        Q2 = np.hstack([Q2, ones])

        q1_pred = Q2.dot(f_projection.T)[:, :2] / Q2.dot(f_projection.T)[:, 2:3]
        q2_pred = Q1.dot(b_projection.T)[:, :2] / Q1.dot(b_projection.T)[:, 2:3]

        residuals = np.vstack([q1_pred.T - q1.T, q2_pred.T - q2.T]).flatten()
        return residuals

    def fusion_reprojection_residuals(self, dof, q1, q2, Q1, Q2, IMU_yaw):
        # Get the rotation vector
        r = dof[:3]
        R, _ = cv2.Rodrigues(r)
        IMU_yaw_radians = IMU_yaw * np.pi / 180
        alpha = 0.5
        fusion_yaw = alpha * IMU_yaw_radians + (1 - alpha) * np.arctan2(R[1, 0], R[0, 0])
        ...
        t = dof[3:]
        fusion_transf = self._form_transf(fusion_R, t)

        # Create the projection matrices
        fusion_f_projection = np.matmul(self.P_l, fusion_transf)
        fusion_b_projection = np.matmul(self.P_l, np.linalg.inv(fusion_transf))
        ...
        fusion_q1_pred = Q2.dot(fusion_f_projection.T)[:, :2] / Q2.dot(fusion_f_projection.T)[:, 2:3]
        fusion_q2_pred = Q1.dot(fusion_b_projection.T)[:, :2] / Q1.dot(fusion_b_projection.T)[:, 2:3]

        fusion_residuals = np.vstack([fusion_q1_pred.T - q1.T, fusion_q2_pred.T - q2.T]).flatten()
        return fusion_residuals

    def get_tiled_keypoints(self, img, tile_h, tile_w):    # 타일링된 키포인트 얻기 : 최적의 키 포인트 10개만 검출
    ...
    def estimate_pose(self, q1, q2, Q1, Q2, IMU_yaw, max_iter=100):
        ...
        min_error = float('inf')
        fusion_min_error = float('inf')
        early_termination = 0

        for _ in range(max_iter):
            # Choose 12 random feature points
            sample_idx = np.random.choice(range(q1.shape[0]), 12)
            sample_q1, sample_q2, sample_Q1, sample_Q2 = q1[sample_idx], q2[sample_idx], Q1[sample_idx], Q2[sample_idx]

            # Make the start guess
            in_guess = np.zeros(6)
            # Perform least squares optimization
            opt_res = least_squares(self.reprojection_residuals, in_guess, method='trf', max_nfev=200,
                                    args=(sample_q1, sample_q2, sample_Q1, sample_Q2))
            fusion_opt_res = least_squares(self.fusion_reprojection_residuals, in_guess, method='trf', max_nfev=200,
                                    args=(sample_q1, sample_q2, sample_Q1, sample_Q2, IMU_yaw))

            # Calculate the error for the optimized transformation
            error = self.reprojection_residuals(opt_res.x, q1, q2, Q1, Q2)
            fusion_error = self.fusion_reprojection_residuals(fusion_opt_res.x, q1, q2, Q1, Q2, IMU_yaw)

            error = error.reshape((Q1.shape[0] * 2, 2))
            error = np.sum(np.linalg.norm(error, axis=1))
            fusion_error = fusion_error.reshape((Q1.shape[0]*2,2))
            fusion_error = np.sum(np.linalg.norm(fusion_error, axis=1))
            ...
            if fusion_error < fusion_min_error:
                fusion_min_error = fusion_error
                fusion_out_pose = fusion_opt_res.x
                early_termination = 0
            else:
                early_termination += 1
            if early_termination == early_termination_threshold:
        ...
        # Get the rotation vector
        r = out_pose[:3]
        fusion_r = fusion_out_pose[:3]

        # Make the rotation matrix
        R, _ = cv2.Rodrigues(r)
        fusion_R, _ = cv2.Rodrigues(fusion_r)

        # Get the translation vector
        t = out_pose[3:]
        fusion_t = fusion_out_pose[3:]

        # Make the transformation matrix
        print(f'Rotation matrix: {R}')
        print(f'Fusion rotation matrix: {fusion_R}')

        # Calculate visual yaw
        visual_yaw = np.arctan2(R[1,0],R[0,0])

        transformation_matrix = self._form_transf(R, t)
        fusion_transformation_matrix = self._form_transf(fusion_R,fusion_t)
...
def main():
    data_dir = 'straight_sequence'  # 데이터 디렉토리 (직진:straight_sequence, 회전:real_sequence)
    ...
    plotting.visualize_paths(estimated_path, fusion_estimated_path, "Stereo Visual Inertia Odometry",
                             file_out=os.path.basename(data_dir) + "_vio.html")
