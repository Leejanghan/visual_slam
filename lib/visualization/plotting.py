...
def visualize_paths(gt_path, pred_path, html_tile="", title="Visual Inertia odometry", file_out="plot.html"):
    ...
    fig1.circle("gtx", "gty", source=source, color="blue", hover_fill_color="firebrick", legend_label="estimated")
    fig1.line("gtx", "gty", source=source, color="blue")

    fig1.circle("px", "py", source=source, color="green", hover_fill_color="firebrick", legend_label="fusion estimated")
    fig1.line("px", "py", source=source, color="green")

    # fig1.multi_line("disx", "disy", source=source, legend_label="Error", color="red", line_dash="dashed")
    # groundTruth 값이 없으므로 삭제
    ...
    show(layout([Div(text=f"<h1>{title}</h1>"),
                 ...
                 [fig1],
                 # 본래 [fig1, fig2] 였으나 error 그래프가 별로 필요하지 않아 삭제함
